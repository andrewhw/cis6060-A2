...
def save_pca_explanation_figure(k_PCA_sufficient, \
	...
	# This call will show the figure on the screen.  Note that in
	# that case this program will stop here until you close the window
	if display:
		plt.show()

def save_pca_scatterplot_figure(X, y, data_dirname, display=False):
...
argparser.add_argument("-d", "--display", action="store_true",
		help="Display (show) the results of any figures created on "
			"the screen.  This will halt the run until the figure is "
			"closed.")
